...
class Relu:
    ...
    def forward(self, x):
        ...
        outputs = np.array(list(map(lambda x: x if x > 0 else 0, x.ravel()))).reshape(x.shape)
        cache = (x, outputs)
    ...
    def backward(self, dout, cache):
        ...
        dx = np.array(list(map(lambda x: 1 if x > 0 else 0, cache[1].ravel()))).reshape(cache[1].shape) * dout
...
def affine_forward(x, w, b):
    ...
    out = np.dot(w.T, x.reshape(N, -1).T).T + b
...
def affine_backward(dout, cache):
    ...
    dx = np.dot(w, dout.T).T.reshape(x.shape)
    dw = np.dot(x.reshape(x.shape[0], -1).T, dout).reshape(w.shape) / x.shape[0]
    db = dout.sum(axis = 0).reshape(b.shape) / x.shape[0]
