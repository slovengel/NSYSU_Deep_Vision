...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        outputs = np.array(list(map(lambda x: x if x > 0 else self.slope * x, x.ravel()))).reshape(x.shape)
        cache = (x, outputs)
    ...
    def backward(self, dout, cache):
        ...
        dx = np.array(list(map(lambda x: 1 if x > 0 else self.slope, cache[1].ravel()))).reshape(cache[1].shape) * dout
