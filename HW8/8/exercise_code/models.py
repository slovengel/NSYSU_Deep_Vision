...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_size, self.hp["n_hidden"]),
            nn.ReLU(),            
            nn.Linear(self.hp["n_hidden"], 784)
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(output_size, self.hp["n_hidden"])
        )
...
class Autoencoder(pl.LightningModule):
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(self.train_set, shuffle=True, batch_size=self.hp['batch_size'])

    def val_dataloader(self):
        return torch.utils.data.DataLoader(self.val_set, batch_size=self.hp['batch_size'])
    ...
    def configure_optimizers(self):
        ...
        self.val_set = torch.tensor(self.val_set)
        optim = torch.optim.Adam([self.train_set, self.val_set], lr=0.0001)
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams, encoder, train_set=None, val_set=None, test_set=None):
        ...
        self.data['train'] = self.forward(torch.flatten(self.data['train'].images, 1).float())
        self.data['val'] = self.forward(torch.flatten(self.data['val'].images, 1).float())
        self.data['test'] = self.forward(torch.flatten(self.data['test'].images, 1).float())
    ...
    def general_step(self, batch, batch_idx, mode):
        ...
        #print(len(batch[0]))
        images = batch
        targets = torch.LongTensor(list(map(list, zip(*batch)))[0])
        print(targets)
        flattened_images = images.view(images.shape[0], -1)
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(self.data['train'], shuffle=True, batch_size=self.hp['batch_size'])

    def val_dataloader(self):
        return torch.utils.data.DataLoader(self.data['val'], batch_size=self.hp['batch_size'])

    def test_dataloader(self):
        return torch.utils.data.DataLoader(self.data['test'], batch_size=self.hp['batch_size'])
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.SGD([{'params': self.model.parameters()}], lr=0.01, momentum=0.9)
